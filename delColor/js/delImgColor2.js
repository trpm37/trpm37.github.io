(function(){
    class delImgColor{
        ...
        init(params){
            let _this=this;
            if(params){
                let {w,h,imgId,canvasId,color,tolerance=_this.tolerance}=params;
                _this.w=w;
                _this.h=h;
                _this.imgId=imgId;
        ...
        //去色
        delColor(params){console.log(params);
            ...
            function compare(current) {
                for (let i = 0; i < 3; i++) {
                    if (!((1 - tolerance) * rgba[i] <= current[i] && (1 + tolerance) * rgba[i] >= current[i])) return false;
                }
                return true;
            }
        }
...
// let canvas = document.querySelector('#outCvs');
...
// debugger
//     cutout(canvas, [39, 176, 60], 0.2); // 对色进行抠除，容差为0.2
// }
